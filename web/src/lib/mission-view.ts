...
export type DispatchReceipt = {
  ...
  status?: "room_ready" | "completed" | "reconciliation_required";
  at_most_once?: boolean;
};

export type HandoffInvitation = {
  room_id: string;
  token: string;
  expires_at: number;
  duplicate_suppressed: boolean;
...
export function deriveRestoredMissionView(
  ...
  const roomReady = lastEvent(events, "following_room_ready");
  const dispatched = roomReady ?? lastEvent(events, "dispatch_completed");
  const replay = lastEvent(events, "handoff_replayed") ?? lastEvent(events, "dispatch_replayed");
  const dispatchFailure = lastEvent(events, "handoff_failed") ?? lastEvent(events, "dispatch_failed");
  ...
  if (dispatched) {
    ...
    dispatch = {
      ...
      provider_status: String(dispatched.evidence?.provider_status ?? ""),
      ...
      status: roomReady ? "room_ready" : "completed",
      at_most_once: true,
    };
